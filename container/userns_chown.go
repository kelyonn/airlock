...
// base:base — the host UID/GID that this container's mapped root resolves
// to, as handed out by allocateUserNSRange (or userNSHostIDOffset itself
// for a container sharing the fallback range). See the call site in
// container.go's Run for why this is necessary at all: an extracted rootfs
// is owned by real root by default, which a mapped-uid child process has no
// write access to.
//
...
func chownForUserNS(root string, base int) error {
	return filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		if cerr := os.Lchown(path, base, base); cerr != nil {
