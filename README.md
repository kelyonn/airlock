...
- **Image signature verification is opt-in, static-key only, and targets cosign's legacy tag-based signature format.** `--verify-key <path-to-cosign.pub>` requires the image to have a valid [cosign](https://github.com/sigstore/cosign) static-key signature from that PEM public key before anything from it is pulled or run — the same mode as `cosign sign --key`/`cosign verify --key`, implemented in `image/signature.go` using nothing beyond Go's standard `crypto/ecdsa`. Two deliberate scope boundaries, not oversights: (1) this does **not** implement Sigstore's keyless flow (Fulcio short-lived certificates + Rekor transparency-log inclusion proofs) — that would mean either depending on the full `sigstore-go` SDK (the same trade-off avoiding `runc`/`libcontainer` already makes elsewhere in this project) or reimplementing a nontrivial PKI + Merkle-log verification stack from nothing; and (2) it targets the older "simple signing" tag-based convention (`sha256-<digest>.sig`), not the OCI 1.1 referrers-index + DSSE-bundle format current `cosign` (3.1.2+) writes by default with no flag to opt back to the legacy format on the *write* side — see "Debugging notes" for how that mismatch was actually found. Without `--verify-key` (the default), layer *integrity* is still checked (SHA-256 against the digest the registry returned), but there's no verification of *provenance* — a compromised registry serving a correctly-hashed malicious layer isn't detected.
...
**Modern `cosign` no longer writes the signature format its own documentation describes.** Building `--verify-key` support meant first confirming the actual on-registry format, not just trusting written references to it — so a real local registry (`registry:2`, with a genuine self-signed TLS cert, since airlock's registry client only ever speaks `https://` by design) got a real image pushed, signed with `cosign sign --key`, and inspected directly over the registry HTTP API. The result didn't match: instead of the documented "simple signing" convention (a `sha256-<digest>.sig`-tagged manifest with a base64 signature in a layer annotation), current `cosign` (3.1.2) writes an OCI 1.1 "referrers" index at `sha256-<digest>` — no `.sig` suffix — pointing at a sub-manifest whose `artifactType` is `application/vnd.dev.sigstore.bundle.v0.3+json`, a DSSE-envelope bundle format entirely different from what got implemented. `--registry-referrers-mode=legacy` turned out to only affect *reads*; there's no flag left to make current `cosign sign` *write* the older format at all. Rather than either quietly shipping code that doesn't match any real `cosign` output, or take on the bundle/DSSE format's real added scope (OCI-referrers discovery, parsing a nested envelope) under time pressure, the decision was to keep the implementation on the older, still-real, still-documented convention — genuinely valid for signatures written by older `cosign` or any tool still targeting it — and verify it properly: the legacy-format artifact was constructed by hand (same ECDSA P-256/PKIX primitives real `cosign` uses, pushed via the registry's actual blob/manifest HTTP API, not mocked), then checked against airlock's own `VerifyCosignSignature` running against that live registry. Five cases — a genuine signature, a corrupted one, one signed for a different image's digest, an entirely unsigned image, and a validly-signed image checked against the wrong key — all resolved correctly. The scope gap (legacy format only, not the current default) is documented in "Security model & limitations" rather than left implicit.
