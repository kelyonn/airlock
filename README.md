...
# Require a valid cosign static-key signature before pulling/running —
# refuses to run an unsigned image, one signed with the wrong key, or one
# whose signature doesn't match what's actually being pulled. See
# "Security model & limitations" for what this does and doesn't cover
# (static-key only, not Sigstore's keyless flow).
airlock run --verify-key ./cosign.pub myregistry.example.com/myimage:tag
```
...
- **Image signature verification is opt-in and static-key only, but supports both on-registry formats.** `--verify-key <path-to-cosign.pub>` requires the image to have a valid [cosign](https://github.com/sigstore/cosign) static-key signature from that PEM public key before anything from it is pulled or run — the same mode as `cosign sign --key`/`cosign verify --key`, implemented using nothing beyond Go's standard `crypto/ecdsa`. Both formats cosign has used are accepted, since both exist in the wild: the OCI 1.1 referrers-index + DSSE-bundle layout current cosign (3.x) writes by default (`image/signature_bundle.go`), and the older tag-based "simple signing" convention from cosign 2.x (`image/signature.go`). They live under distinct tags (`sha256-<digest>` and `sha256-<digest>.sig`), so trying both is unambiguous rather than guesswork; if neither verifies, both failures are reported, because "no artifact of this kind exists" and "a signature exists but doesn't verify" are very different answers. The remaining deliberate scope boundary: this does **not** implement Sigstore's keyless flow (Fulcio short-lived certificates + Rekor transparency-log inclusion proofs), which would mean either depending on the full `sigstore-go` SDK — the same trade-off avoiding `runc`/`libcontainer` already makes elsewhere here — or reimplementing a nontrivial PKI + Merkle-log verification stack from nothing. Without `--verify-key` (the default), layer *integrity* is still checked (SHA-256 against the digest the registry returned), but there's no verification of *provenance* — a compromised registry serving a correctly-hashed malicious layer isn't detected.
...
**Modern `cosign` no longer writes the signature format its own documentation describes.** Building `--verify-key` support meant first confirming the actual on-registry format, not just trusting written references to it — so a real local registry (`registry:2`, with a genuine self-signed TLS cert, since airlock's registry client only ever speaks `https://` by design) got a real image pushed, signed with `cosign sign --key`, and inspected directly over the registry HTTP API. The result didn't match: instead of the documented "simple signing" convention (a `sha256-<digest>.sig`-tagged manifest with a base64 signature in a layer annotation), current `cosign` (3.1.2) writes an OCI 1.1 "referrers" index at `sha256-<digest>` — no `.sig` suffix — pointing at a sub-manifest whose `artifactType` is `application/vnd.dev.sigstore.bundle.v0.3+json`, a DSSE-envelope bundle format entirely different from what got implemented. `--registry-referrers-mode=legacy` turned out to only affect *reads*; there's no flag left to make current `cosign sign` *write* the older format at all. Rather than quietly ship code matching no real `cosign` output, the first pass kept the implementation on the older, still-real convention and documented the gap explicitly instead of leaving it implicit.

That gap is now closed: the bundle format is implemented too (`image/signature_bundle.go`), and both are tried. Building it meant establishing the format empirically all over again rather than from documentation — the same registry was stood back up, a real image signed with real `cosign` 3.1.2, and every layer walked by hand: fallback tag → OCI index → the manifest whose `artifactType` is `application/vnd.dev.sigstore.bundle.v0.3+json` → its layer blob → the DSSE envelope inside. The one genuinely subtle part is what DSSE actually signs. It is *not* the payload bytes (that's the legacy format's rule) but a "pre-authentication encoding" — `DSSEv1 <len(payloadType)> <payloadType> <len(payload)> <payload>` — which binds the payload to its declared content type so an envelope can't be reinterpreted as a different type. That construction was confirmed against the real signature with `openssl dgst -verify` *before* any Go was written, so the implementation started from a known-correct target instead of a plausible guess. The captured bundle and its public key are now checked in as test fixtures (`image/testdata/`), which makes the interop test a permanent regression guard against genuine cosign output rather than a one-off manual check — including an assertion that the signature does *not* verify against the bare payload, pinning the PAE behavior that would otherwise be easy to "simplify" away later.
